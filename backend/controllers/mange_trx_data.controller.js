import { transactionsDB, initializeTransactionsDB, itemsDB } from "../config/database.js";
...
export const upsertSession = async (req, res) => {
     ...
     try {
          ...
          try {
               ...
               return res.status(200).json({ status: true, message: "Session updated", data: result });
          } catch (err) {
          ...
          res.status(201).json({ status: true, message: "Session created", data: result });
     } catch (err) {
          res.status(500).json({ status: false, message: "Error storing session", error: err.message });
     }
};
...
export const getSessionByDate = async (req, res) => {
     ...
     try {
          ...
          res.status(200).json({ status: true, message: "Session retrieved", data: doc });
     } catch (err) {
          if (err.status === 404) {
...
export const getSessionsByRange = async (req, res) => {
     ...
     try {
          ...
          res.status(500).json({ status: false, message: "Error fetching sessions", error: err.message });
     }
};
...
export const deleteSession = async (req, res) => {
     ...
     try {
          ...
          res.status(200).json({ status: true, message: "Session deleted", data: result });
     } catch (err) {
